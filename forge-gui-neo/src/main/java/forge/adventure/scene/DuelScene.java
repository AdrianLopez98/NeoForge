// COPIA SOMBRA de forge-gui-mobile/src/forge/adventure/scene/DuelScene.java (NeoForge).
// Va antes en el classpath y sustituye a la de Forge; el original NO se toca.
// Dos cambios: el bloque 'NEOFORGE' de enter() y 'NEOFORGE-2' en GameEnd().
// Si Forge cambia el original,
...
public class DuelScene extends ForgeScene {
    ...
    public void GameEnd() {
        ...
        try {
            // ---- NEOFORGE-2: el ganador se pregunta al MATCH, no al juego ----
            // Segundo (y ultimo) cambio frente al DuelScene de Forge. Con nuestra
            // mesa, esto corre DESPUES de que HostedMatch.endCurrentGame() haya
            // puesto el juego a null (es lo que avisa a la interfaz de que el
            // duelo acabo), asi que getGame() era null: saltaba un NPE, lo
            // atrapaba el catch de abajo y el duelo contaba como DERROTA aunque
            // hubieras ganado — te echaba de la mazmorra y no cobrabas nada
            // (17-09-2026). getMatch() sigue ahi y responde lo mismo:
            // getGame().getMatch() ES ese match.
    ...
    public void enter() {
        ...
        // ---- NEOFORGE: el combate se juega en NUESTRA mesa ----
        // Unico cambio frente al DuelScene de Forge. Todo lo de arriba (mazos,
        // objetos, bendiciones, efectos de mazmorra, vida) es suyo sin tocar.
        if (forge.neo.adventure.NeoDuelBridge.enabled()) {
            forge.neo.adventure.NeoDuelBridge.play(hostedMatch, rules, appliedVariants, players,
                    humanPlayer, enemy.getName(), () -> {
                        GameEnd();
                        if (!hasCallbackExit())
                            exitDuelScene();
                    });
